...
Backend reviews additionally produce `architecture.svg`. Their smaller manifest records the backend
summary, full commit SHAs, and generation time, but does not contain the web manifest's config,
browser, command, or artifact-hash provenance.
...
Evidence section, and posts that image-bearing version. **The skill does not edit the PR
description.**
...
Only examples under **Available now, v0.5.0** describe executable behavior in this release.
