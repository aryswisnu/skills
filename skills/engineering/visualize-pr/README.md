...
words; the sequence diagram, as the first piece of evidence; one line of numbers; then the change
map last. For a one-file change the whole generated part is that line plus the two diagrams. Here is a real one, a Bitbucket PR that changed
...
The bullets and the pseudocode came from the agent through `--notes`, no prose, no restated
statistics; the sequence diagram came through `--diagram`. Fenced, not indented: after a bullet
