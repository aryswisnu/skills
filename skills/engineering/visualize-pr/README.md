...
Backend reviews additionally produce `architecture.svg` (and, with `--post-comment`, a rasterized
`architecture.png`). Their smaller manifest records the backend summary, full commit SHAs, and
...
| **Backend / non-web** | `--backend` analyzes the diff and emits a change summary plus an editorial architecture diagram (`architecture.svg`), no browser or config required. With `--post-comment`, the map is rasterized to PNG and embedded in the comment. |
...
Evidence section, and posts that image-bearing version. For backend reviews, `--post-comment`
rasterizes the `architecture.svg` change map to PNG and embeds it the same way. **The skill does not
edit the PR description.**

![Real side-by-side evidence generated by the CLI](docs/example-pr-comment-side-by-side.png)

Backend reviews embed a rasterized change map in the same way:
...
Only examples under **Available now, v0.7.0** describe executable behavior in this release.
