...
test('buildBackendComment carries no summary heading, no SVG pointer, and no Evidence section without an image', () => {
  const summary = summarizeChange(parseNameStatus(nameStatus), parseNumstat(numstat));
  ...
  assert.doesNotMatch(md, /## Change summary/);
  assert.doesNotMatch(md, /architecture\.svg/);
  assert.doesNotMatch(md, /## Evidence/);
  assert.match(md, /> Generated by `visualize-pr`\. Evidence for a reviewer, not an approval\.$/);
});

test('buildBackendComment embeds an image under Evidence when an image URL is supplied', () => {
  const summary = summarizeChange(parseNameStatus(nameStatus), parseNumstat(numstat));
  const pr = { title: 'Refactor billing', baseRef: 'main', headRef: 'feature/billing' };
...
test('buildBackendComment is compact: no repeated summary heading or SHAs, files inline when few', () => {
  const summary = summarizeChange(parseNameStatus('M\tsrc/controllers/agentStats.controller.js\n'), parseNumstat('110\t20\tsrc/controllers/agentStats.controller.js\n'));
  const pr = { title: 'T', baseRef: 'development', headRef: 'AI-4176', number: 1 };
  const md = buildBackendComment(summary, 'a'.repeat(40), 'b'.repeat(40), pr);
  assert.doesNotMatch(md, /## Change summary/);
  assert.equal((md.match(/aaaaaaa/g) || []).length, 1, 'the SHA appears once, on the title line');
  assert.match(md, /^1 file changed \(\+110 -20\): `src\/controllers\/agentStats\.controller\.js`$/m);
  assert.doesNotMatch(md, /### By module/, 'one module needs no table');
  assert.doesNotMatch(md, /### Most changed files/, 'one file needs no ranking');
  assert.match(md, /> Generated by `visualize-pr`\. Evidence for a reviewer, not an approval\.$/);
});

test('buildBackendComment keeps the module table and ranking when they carry information', () => {
  const ns = parseNameStatus('M\tsrc/a.js\nA\tsrc/b.js\nM\ttest/c.js\nM\ttest/d.js\n');
  const num = parseNumstat('9\t1\tsrc/a.js\n5\t0\tsrc/b.js\n2\t2\ttest/c.js\n1\t0\ttest/d.js\n');
  const md = buildBackendComment(summarizeChange(ns, num), 'a'.repeat(40), 'b'.repeat(40), { title: 'T', baseRef: 'm', headRef: 'f', number: 1 });
  assert.match(md, /^4 files changed \(\+17 -3\)$/m);
  assert.match(md, /### By module/);
  assert.match(md, /### Most changed files/);
});

test('buildBackendComment places --notes right after the title, before the change line', () => {
  const summary = summarizeChange(parseNameStatus('M\tsrc/a.js\n'), parseNumstat('1\t1\tsrc/a.js\n'));
  const md = buildBackendComment(summary, 'a'.repeat(40), 'b'.repeat(40), { title: 'T', baseRef: 'm', headRef: 'f', number: 1 }, null, null, null, '- accept arrays\n\n```\nfor v in values: expand(v)\n```');
  const title = md.indexOf('**T**');
  const notes = md.indexOf('- accept arrays');
  const change = md.indexOf('1 file changed');
