...
5. Show the draft and ask. In Claude Code the four choices (comment, description, both, not now)
   ...
   harness's own approval prompt is the button. On approval, `--publish <dir>` posts the draft in
   under a second. Nothing is recomputed, so what you read is what lands; a GitHub web review
   uploads and embeds its screenshots at that moment.
